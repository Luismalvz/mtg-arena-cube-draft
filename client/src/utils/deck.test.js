...
test('only tokens generated by cards in the main deck are included and they do not count toward forty', () => {
  const tokenPicks = [
    {instanceId:'warboss',name:'Legion Warboss'},
    {instanceId:'rakdos',name:'Rakdos Guildmage'},
    {instanceId:'sideboard',name:'March of the Multitudes'}
  ];
  const deck = buildDeck(tokenPicks, ['warboss','rakdos'], {'gk1-100':38}, lands);
  assert.deepEqual(deck.tokens.map(token => token.id), ['trvr-8','trvr-9']);
  assert.equal(deck.count, 40);
  assert.match(deckText(deck), /Tokens\n1 Goblin \(trvr\) 8\n1 Goblin \(trvr\) 9\n$/);
  assert.doesNotMatch(deckText(deck), /Soldier \(tgrn\) 2/);
});
test('shared token types are deduplicated while distinct Spirit printings remain separate', () => {
  const tokenPicks = [
    {instanceId:'eyes',name:'Eyes in the Skies'},
    {instanceId:'screech',name:'Battle Screech'},
    {instanceId:'afterlife',name:'Ministrant of Obligation'},
    {instanceId:'teysa',name:'Teysa, Orzhov Scion'}
  ];
  const deck = buildDeck(tokenPicks, tokenPicks.map(card => card.instanceId), {}, lands);
  assert.deepEqual(deck.tokens.map(token => token.id), ['trvr-1','trna-10','trvr-4']);
});
